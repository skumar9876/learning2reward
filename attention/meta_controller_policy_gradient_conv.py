...
class PolicyEstimator():
    ...
    def __init__(self, learning_rate=0.01, scope="policy_estimator"):
        with tf.variable_scope(scope):
            ...
            self.W_conv1 = weight_variable([3, 3, 1, 2])
            self.b_conv1 = bias_variable([2])
            ...
            self.W_conv2 = weight_variable([3, 3, 2, 1])
            ...
            self.image_flat = tf.reshape(self.h_pool2, [-1, 5*5*1])

            hidden_layer_size = 5

            # Hidden layer for the image input
            self.W_fc1 = weight_variable([1*5*5, hidden_layer_size])
            self.b_fc1 = bias_variable([hidden_layer_size])
            self.h_fc1 = tf.tanh(tf.matmul(self.image_flat, self.W_fc1) + self.b_fc1) #ADDED TANH HERE
            ...
            self.h_fc_sent = tf.tanh(tf.matmul(self.sentence, self.W_fc_sent) + self.b_fc_sent) #ADDED TANH HERE!!!

            print self.h_fc_sent.get_shape()

            self.h_fc_concatenated = tf.concat_v2([self.h_fc1, self.h_fc_sent], 1)



            # Second hidden layer
            self.W_fc2 = weight_variable([2*hidden_layer_size, 4])
            self.b_fc2 = bias_variable([4])

            self.h_fc2 = tf.tanh(tf.matmul(self.h_fc_concatenated, self.W_fc2) + self.b_fc2) #ADDED TANH HERE!!!
...
def reinforce(env, estimator_policy, num_episodes):
    ...
    for i_episode in range(num_episodes):     
        ...
        loss, loss_entropy = estimator_policy.update(images, sentences, reward_arr, chosen_rooms)
