...
class StateProcessor(object):
  ...
  def init_weights:
    ...
    self.W_fc1 = weight_variable([self.image_size*self.image_size, 100])
    self.b_fc1 = bias_variable([100])

    # Second hidden layer 
    self.W_fc2 = weight_variable([100 + self.sentence_length, 100])
    self.b_fc2 = weight_variable([100])


  def __call__(self, image, sentence)
    '''
    Inputs:
    [1, 5, 5] size input image
    [1, 5] sentence
    '''

    image_flat = tf.reshape(image, [1, self.image_size*self.image_size])

    # First hidden layer
    h_fc1 = tf.matmul(image_flat, self.W_fc1) + self.b_fc1

    # Concatenate the sentence
    h_fc1_concatenated = tf.concat_v2([h_fc1, sentence], 1)

    # Second hidden layer
    h_fc2 = tf.matmul(h_fc1_concatenated, self.W_fc2) + self.b_fc2
